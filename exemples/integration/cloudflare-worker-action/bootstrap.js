import"./../exemples/integration/chunk-3m06d5g0.js";
...
// cloudflare-client-bootstrap:/home/shpaw415/frame-master-plugins/cloudflare-pages-dynamic-ssr/exemples/integration/node_modules/frame-master-plugin-cloudflare-pages-functions-action/src/bootstrap.ts
function InitActionData(...props) {
  ...
  for (const prop of props) {
    if (prop instanceof FormData) {
      throw new Error("only one prop is permitted with a FormData in a Worker Action");
      ...
      if (!(prop[0] instanceof File)) {
        ...
        prop.forEach((p) => {
          if (p instanceof File) {
            ...
            throw new Error("only File instances are permitted inside an array of Files in a Worker Action");
          }
        });
      }
    } else {
...
async function makeActionRequest(props, pathname, method = "POST", fetcher = fetch) {
  if (method === "GET" || method === "HEAD") {
    if (props.length > 0) {
      ...
      for (const prop of props) {
        i = i++;
        url.searchParams.append(`arg_${i}`, encodeURIComponent(JSON.stringify(prop)));
      }
      pathname = url.toString();
      const res2 = await fetcher(pathname, {
        method,
        headers: {
      ...
      return await ParseServerActionResponse(res2);
    }
  }
...
async function ParseServerActionResponse(response) {
  ...
  if (!response.ok && dataType !== "response")
    throw new Error(`error when Calling worker action ${response.url}: ${response.statusText}`);
  let blob = null;
  let fileData = null;
  switch (dataType) {
    case "json":
      return dist_default.parse(await response.text());
    case "blob":
      return await response.blob();
    case "file":
      blob = await response.blob();
      fileData = JSON.parse(response.headers.get("fileData") || "");
    ...
    default:
      try {
        return await dist_default.parse(await response.text());
      } catch (e) {
        throw new Error(`Unsupported data type returned from server action: ${response.headers.get("dataType")}`, { cause: e });
      }
  }
}
var bootstrap_default = makeActionRequest;
export {
  bootstrap_default as default
};

//# debugId=8F12D38CF203387764756E2164756E21
//# sourceMappingURL=./../../cloudflare-worker-action/bootstrap.js.map
